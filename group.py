...
def fetch_group_info(group_input):
    ...
    try:
        # FIXED: await coroutine properly
        entity = tele_client.loop.run_until_complete(tele_client.get_entity(inp))
    ...
    try:
        msgs = tele_client.loop.run_until_complete(tele_client.iter_messages(entity, reverse=True, limit=1).__anext__())
        if msgs and getattr(msgs, "date", None):
